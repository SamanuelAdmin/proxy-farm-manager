...
class NetworksManager:
    ...
    def getAllInterfaces(self) -> list[NetworkInterface]:
        ...
        for devicePath in self.__networkDbusInterface.GetAllDevices():
            ...
            interfaceData: Optional[NetworkInterface] = self.getInterfaceData(
                deviceObject
            )

            if interfaceData is None:
                continue
    ...
    def createBridge(
        self, childrenIfaces: list[NetworkInterface], name="proxy-inner"
    ) -> BridgeInterface:
        """
        ...
        # checking if iface has been already created
        try:
            ipr = IPRoute()
